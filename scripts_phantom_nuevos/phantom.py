...
async def main():
    ...
    discord.send_ingreso("Phantom Nuevos")
    ...
    try:
        ...
        discord.send_webhook(sesion_actual, meta, "Phantom Nuevos")
        if _mac_backup:
            _mac_backup.export_session()

    except KeyboardInterrupt:
        print(f"\n\n🛑 PROCESO DETENIDO POR EL USUARIO.")
        discord.send_webhook(sesion_actual, meta, "Phantom Nuevos", is_interrupted=True)
        if _mac_backup:
            _mac_backup.export_session()
            
    except Exception as e:
        print(f"\n\n🚨 ERROR CRÍTICO: {e}")
        discord.send_error("Phantom Nuevos", str(e))
