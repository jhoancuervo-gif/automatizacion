# =========================================================
# SCRIPT: poeport81G.ps1 - DOBLE AUTENTICACIÓN
# =========================================================
...
do {
    ...
    Write-Host "   HELLOTEK - CARGA SNMP (INTENTO DOBLE LOGIN)           " -ForegroundColor Cyan
    Write-Host "==========================================================" -ForegroundColor Cyan
    ...
    $authList = @("admin:admin", "admin:somos123.")
    $validAuth = $null

    foreach ($auth in $authList) {
        # Intentamos una petición pequeña (el index) para validar el usuario/clave
        $test = curl.exe -u $auth -s -o NUL -w "%{http_code}" "http://$ip/index.htm" --max-time 2
        if ($test -eq "200") {
            $validAuth = $auth
    ...
    if (-not $validAuth) {
        Write-Host " [ERROR: Clave incorrecta]" -ForegroundColor Red
        Write-Host " No se pudo ingresar con 'admin' o 'somos123.'"
        $next = Read-Host "ENTER para reintentar / 'S' para salir"
        if ($next -eq "s") { break } else { continue }
    }

    # --- PASO 3: CARGA Y VERIFICACIÓN ---
    Write-Host "[3/3] Enviando Configuración..." -NoNewline
    
    # Usamos la credencial que funcionó en el paso anterior
