...
# =====================================================================
# CONFIGURACIÓN DEL SISTEMA (V33 - PRODUCTION BATCH EDITION)
# =====================================================================
...
MAX_WORKERS = 10  # Limite para evitar saturacion de red en lotes grandes
...
def subir_archivo_resiliente(session, url, file_path, prefix):
    """Reintenta la subida si hay saturacion de red"""
    for i in range(2):
...
def flash_process_production(dev):
    ip = dev['ip']
    prefix = f"[{ip}] "
    # Pequeño jitter para no golpear la red todos al mismo tiempo
    ...
    try:
        ...
        if subir_archivo_resiliente(s, f"http://{ip}/cgi/SG1008.bin", CONFIG_FILE, prefix):
            # Pausa de seguridad post-config mas larga para lotes grandes
            time.sleep(4)
            esperar_pulso_reinicio(ip, prefix, max_down=8)
        else: return False

        # 3. Cambio de Clave (Con Reintentos)
        ...
        log(f"{prefix}FALLO FINAL DE CLAVE.", Colors.RED)
        return False
    except: return False
...
def main():
    ...
    print(f"{Colors.CYAN}==========================================================")
    print(f"   HELLOTEK - V33 (PRODUCTION BATCH)                     ")
    print(f"=========================================================={Colors.END}")
    ...
    while len(discovered) < target:
        ...
        for pwd in PASSWORDS:
            try:
                r = requests.get(f"http://{BASE_IP}/index.htm", auth=("admin", pwd), timeout=1.5)
        ...
        if auth:
            subprocess.run(["ping", "-n", "1", "-w", "400", BASE_IP], capture_output=True)
            ...
            if mac and not any(d['mac'] == mac for d in discovered):
                ...
                try: requests.post(f"http://{BASE_IP}/cgi/sysipset.cgi", auth=auth, data=payload, timeout=5)
                except: pass
                
                # Esperar a que el equipo aparezca
                start_wait = time.time()
                while (time.time() - start_wait) < 15:
                    if verify_http(temp_ip):
    ...
    log(f"\n--- FASE 2: CONFIGURACIÓN SEGURA (MAX 10 SIMULTÁNEOS) ---", Colors.BOLD)
    # Limitamos workers para estabilidad de red
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        results = list(executor.map(flash_process_production, discovered))
